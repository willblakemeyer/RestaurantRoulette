...
function draw() {
  ...
  if (doSpin) {
    ...
    angleVel =
      Math.floor(
        Math.min(2, angleVel / ((angleMultAccel - 1) / 4 + 1)) * 100000
      ) / 100000;
    if (angleVel <= 0) {
...
function drawWheel() {
  let numSegments = restaurantStars.reduce(
  ...
  for (let i = 0; i < restaurantNames.length; i++) {
    ...
    if (
      (angle + rotation * segmentAngle + readOffset) % twoPI >
      (angle + (rotation + restaurantStars[i]) * segmentAngle + readOffset) %
        twoPI
    ) {
    ...
    arc(
      ...
      (rotation + restaurantStars[i]) * segmentAngle,
      PIE
    );
    rotation += restaurantStars[i];
  }
